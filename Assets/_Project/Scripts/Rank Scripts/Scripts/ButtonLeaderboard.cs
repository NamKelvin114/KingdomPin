...
namespace Pancake.GameService
{
    ...
    public class ButtonLeaderboard : MonoBehaviour
    {
        ...
        public void Setup()
        {
            // canvas = PopupController.Instance.Canvas;
            popupEnterName = PopupController.Instance.GetComponentInChildren<PopupEnterName>(true);
            popupLeaderboard = PopupController.Instance.GetComponentInChildren<PopupLeaderboard>(true);
            popupNotification = PopupController.Instance.GetComponentInChildren<PopupNotification>(true);
            popupNoInternet = PopupController.Instance.GetComponentInChildren<PopupNoInternet>(true);
        }
        ...
        private void AuthServiceOnLoginSuccess(LoginResult result)
        {
            ...
            LoginResultModel.Init(r.PlayerId, r.DisplayName, countryCode,
                r.LinkedAccounts.Any(_ => _.Platform == LoginIdentityProvider.Facebook));
    ...
    public class ButtonLeaderboardEdior : UnityEditor.Editor
    {
        ...
        private void OnEnable()
        {
            _buttonLeaderboard = target as ButtonLeaderboard;
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                Sprite first =
                    (Sprite)AssetDatabase.LoadAssetAtPath("Packages/com.pancake.ui/Runtime/Fetch/Sprites/01.png",
                        typeof(Sprite));
                if (first == null)
                    first = (Sprite)AssetDatabase.LoadAssetAtPath("Assets/_Root/Runtime/Fetch/Sprites/01.png",
                        typeof(Sprite));
                ...
                if (animator.runtimeAnimatorController == null)
                {
                    animator.runtimeAnimatorController =
                        (UnityEditor.Animations.AnimatorController)AssetDatabase.LoadAssetAtPath(
                            "Assets/_Root/Runtime/Fetch/Animation/FetchAnimator.controller",
                            typeof(UnityEditor.Animations.AnimatorController));
                }
